...
zaznam = "/Users/peterbrenkus/Desktop/skola/sem3/pks/Z1/pcap/z1-final_v2.pcap"
...
for packet in packets:          #na zaciatku prejde vsetky pakety a zapise informacie do paketyAll.yaml
    hex_data = packet.build().hex()         #vyrobi aj ip statistiku
    ...
    packet_info["len_frame_pcap"] = len(packet.payload) #zapise dlzku paketu od pcap API
    manual_len = 0
    for layer in packet:
    ...
    if int(EternetType, 16) > 1500:                 #ak je EthernetType vacsi ako 1500, tak je to Ethernet II
        ...
        if packet_info["ether_type"] == "IPv4":
            ...
            SIP = ""
            for i in range(44, 76, 4):      #zapise Source IPv6
            ...
            SIP = SIP[0:4] + ":" + SIP[4:]
            DIP = ""
            for i in range(76, 108, 4):         #zapise Destination IPv6
                part = f"{hex_data[i:i + 4]}"
                if part[0] == "0":
                    part = part[1:]
                DIP = DIP + part + ":"
...
if filter == "HTTP" or filter == "HTTPS" or filter == "Telnet" or filter == "SSH" or filter == "FTP-control" or filter == "FTP-data":
    tcp_pakety = []         #potom podla filtra spracuje iba pakety so zadanym protokolom
    for packet in packets:              #sracovanie je rovnke ako predtym
    ...
    communication = {}
    zaciatky = []
    for index in range(len(tcp_pakety)):        #vytiahne zaciatky komunikacii (flag SYN)
        if "SYN" in tcp_pakety[index]["tcp_flags"] and "ACK" not in tcp_pakety[index]["tcp_flags"]:
            ...
            communication["packets"] = []
    comms = []
    for zaciatok in zaciatky:           #pre kazdy zaciatok potom prida prislusne pakety z komunikacie
    ...
    complete_comms = []
    incomplete_comms = []
    for comm in zaciatky:   #pozera ci je komunikacia kompletna (flag SYN, ACK, SYN, ACK, ... FIN, ACK, FIN, ACK) a ich dalsie kombinacie
        zaciatok = False
        koniec = False
        del comm["packets"][0]
    ...
    for comm in complete_comms:     #zoradi data v komunikaciach
        komunikacia = {}
    ...
    incomplete_comm = {}
    loop = False
    for packet in tcp_pakety:       #spracuje nekompletne komunikacie
       if "app_protocol" in packet:
        if packet["app_protocol"] == filter:
            incomplete_comm["number_comm"] = 1
            incomplete_comm["packets"] = []
            incomplete_comm["packets"].append(packet)
            loop = True
            break
    if len(tcp_pakety) > 0 and loop:
        src_port = incomplete_comm["packets"][0]["src_port"]
        dst_port = incomplete_comm["packets"][0]["dst_port"]
        tcp_pakety.remove(incomplete_comm["packets"][0])
        for packet in tcp_pakety:
            if packet["src_port"] == src_port and packet["dst_port"] == dst_port:
                incomplete_comm["packets"].append(packet)
            elif packet["src_port"] == dst_port and packet["dst_port"] == src_port:
                incomplete_comm["packets"].append(packet)
        cleaned_comm = incomplete_comm.copy()
        cleaned_comm["packets"] = []
        for packet in incomplete_comm['packets']: #vycisti nadbytocne data z komunikacii
    ...
    for comm in complete_comms_sorted:  #vycisti nadbytocne data z komunikacii
    ...
    complete_final = []
    incomplete_final = []
    if filter == "HTTP":        #podla filtra vyberie komunikacie s danym protokolom
    ...
    with open("output.yaml", "w") as file:  # vypise do yaml
        ruamel.yaml.dump(output, file, Dumper=ruamel.yaml.RoundTripDumper)

elif filter == "TFTP":
    udp_pakety = []
    counter = 1
    for packet in packets:      #spracuje iba UDP pakety
        hex_data = packet.build().hex()
        EternetType = hex_data[24:28]
        if EternetType == "0800":
            ip_type = hex_data[46:48]
    ...
    for paket in udp_pakety:
        ...
        if dst_port == "0045":
            paket["protocol"] = "TFTP"
            tftp_pakety.append(paket)    #spracuje iba TFTP pakety
    for paket in udp_pakety:
    ...
    server_port = ""
    client_port = ""
    for zaciatok in tftp_pakety:        # spracuje komunikacie
    ...
    complete_comms = []
    incomplete_comms = []
    for comm in comms:    #rozdeli komunikacie na kompletne a nekompletne
        if comm["packets"][-2]["tftp_opcode"] == 3 and comm["packets"][-1]["tftp_opcode"] == 4:
            complete_comms.append(comm)
        else:
            incomplete_comms.append(comm)
    complete_comms_clean = []
    for comm in complete_comms: #vycisti nadbytocne data z komunikacii
        cleaned_comm = comm.copy()
    ...
    incomplete_comms_clean = []     #vycisti nadbytocne data z komunikacii
    ...
    ip_pakety = []
    counter = 1
    for packet in packets:      #spracuje iba ICMP pakety
        hex_data = packet.build().hex()
        EternetType = hex_data[24:28]
        if EternetType == "0800":
            ...
            if protocolID == "01":
                ...
                if icmp_type == "08":
                    ...
                    packet_info["icmp_id"] = int(hex_data[132:136], 16)
                    packet_info["icmp_seq"] = int(hex_data[136:140], 16)
                    counter2 = 1
                    hexDataFinal = ""
                    for pismeno in hex_data:
                        hexDataFinal += pismeno
    ...
    counter3 = 1
    ids = []
    for request in icmp_requests:   #vytiahne ID jednotlivych komunikacii
        icmp_id = request["icmp_id"]
        if ids.__contains__(icmp_id):
            continue
        else:
            ids.append(icmp_id)
    for id in ids:  # pre kazdu komunikaciu priradi prisluhajuce pakety
    ...
    complete_comms_clean = []
    incomplete_comms_clean = []
    for comm in complete_comms:    #vycisti kompletne komunikacie z pola paketov
       for packet in comm["packets"]:
            if packet in icmp_requests:
                icmp_requests.remove(packet)
            elif packet in icmp_responses:
                icmp_responses.remove(packet)
    for request in icmp_requests:       #spracuje nekompletne komunikacie
    ...
    for comm in complete_comms:     #vycisti nadbytocne data z komunikacii
        ...
        cleaned_comm["packets"] = [packet.copy() for packet in comm['packets']]
        complete_comms_clean.append(cleaned_comm)
    for comm in incomplete_comms:       #vycisti nadbytocne data z komunikacii
        cleaned_comm = comm.copy()
        incomplete_comms_clean.append(cleaned_comm)
    packets = rdpcap(zaznam)
    counter = 1
    zaciatky = []
    for packet in packets:      #riesi fragmentaciu, spracuje ICMP pakety
        hex_data = packet.build().hex()
        EternetType = hex_data[24:28]
        if EternetType == "0800":
            protocolID = hex_data[46:48]
    ...
    counter4 = 1
    paketyComm = []
    for zaciatok in zaciatky:       #pre kazdy fragmentovany paket priradi fragmenty
    ...
    incomplete_comms = []
    counter = 1
    for packet in packets:    #spracuje iba ARP pakety
        hex_data = packet.build().hex()
        EternetType = hex_data[24:28]
    ...
    communication = {}
    counter3 = 1
    for request in arp_requests:    #priradi odpovede k requestom
        for reply in arp_replies:
            if reply["dst_mac"] == request["src_mac"] and reply["flag"] == "unused":
                ...
                counter3 += 1
                break
    for comm in complete_comms:    #vycisti nadbytocne data z komunikacii, kompletne komunikacie zo zoznamu paketov
        for packet in comm["packets"]:
    ...
    for request in arp_requests:    #zvysne requesty a reply prida do nekompletnych komunikacii
        incomplete_comms.append(request)
    for reply in arp_replies:
        incomplete_comms.append(reply)
    complete_comms_clean = []
    ...
    for packet in packets:  # sracovanie je rovnke ako predtym
        ...
        counter += 1
    counter2 = 0
    for paket in tcp_pakety:    #spracuje iba pakety s flagom FIN
        if "FIN" in paket["tcp_flags"]:
            paketyFin.append(paket)
            counter2 += 1
    for paket in paketyFin:     #vycisti nadbytocne data z paketov
        if "tcp_flags" in paket:
            del paket["tcp_flags"]
    #print(counter2)
    fileOutput = {"name": "PKS2023/24", "pcap_name": "all.pcap", "packets": paketyFin, "number_packets": len(paketyFin)}
    with open("output.yaml", "w") as file:
        ruamel.yaml.dump(fileOutput, file, default_style='', default_flow_style=False, Dumper=ruamel.yaml.RoundTripDumper)